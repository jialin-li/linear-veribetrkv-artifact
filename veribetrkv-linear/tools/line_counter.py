...
parser.add_argument("--mode", dest="mode", help="count: count one file. report: gather a report for this file and its dependencies", required=True, choices=["count", "report", "raw"])
...
def raw(input):
    # generate a cleaned up version of the file, free of comments and other dafny outputs
    counter = line_count_lib.Counter(".")
    dafnyFile = line_count_lib.DafnyFile(input, 0.0)
    counter.collect_clean_files([dafnyFile])
