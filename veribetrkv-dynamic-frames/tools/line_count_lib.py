...
class Counter:
    ...
    def get_clean_file(self, show_ghost, dafny_file):
      tmp_dir = self.make_tmp_dir()
      dafny_dir = os.path.dirname(dafny_file)
      dafny_base = os.path.basename(dafny_file)
      tmp_filename = "%s-%s" % (dafny_base, ("ghost" if show_ghost else "real"))
      tmp_file = os.path.join(tmp_dir, tmp_filename)
    ...
    def collect_clean_files(self, dafny_files):
      for f in dafny_files:
        self.get_clean_file(True, f.filename)
        if not f.is_spec():
          self.get_clean_file(False, f.filename)
